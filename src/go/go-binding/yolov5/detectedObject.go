...
import (
	...
	"github.com/openvinotoolkit/model_sever/src/go/go-binding/utilities"

	"github.com/openvinotoolkit/model_sever/src/go/go-binding/tensorOutput"
	"gocv.io/x/gocv"
)
...
func (dObjs *DetectedObjects) Postprocess (outputs tensoroutput.TensorOutputs, camWidth float32, camHeight float32) error {
	regionCoordsCount := 3
	regionNum := 3
	original_im_w := camWidth
	original_im_h := camHeight
	for _, tensor := range outputs.GetOutputs() {
